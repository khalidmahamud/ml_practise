...
class DataLoader:
    ...
    def preprocess_data(self):
        ...
        encoded = pd.get_dummies(
            self.data[categorical_cols], prefix=categorical_cols
        )

        numeric_columns = encoded.columns
        for col in numeric_columns:
            encoded[col] = pd.to_numeric(encoded[col], errors="coerce")

        boolean_columns = encoded.select_dtypes(include=["bool"]).columns
        encoded[boolean_columns] = encoded[boolean_columns].astype(int)
    ...
    def oversample(self, X_train, y_train):
        ...
        for col in X_train.columns:
            x_over[col] = x_over[col].astype(X_train[col].dtype)
